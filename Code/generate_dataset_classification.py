    ...
    for (exon_boundary_interval, exon_boundary, var) in zip(exon_boundary_intervals, exon_boundaries,
        ...
        for gene in nonoverlapping_gene_intervals:
            if exon_boundary_interval in gene:
                print('In gene!', exon_boundary_interval)
...
def write_to_file(data, file_name):
    ...
    :return: None
    '''
    print(data_path + 'classification/' + file_name)
    with open(data_path + 'classification/' + file_name, "w+") as file:
...
def manipulate(dataset):
    ...
    for gene in data[0:3]:
        ...
        if (len(exons_ranges_in_transcript) >= 1):
            ...
            # POSITIVE SAMPLES
            exon_start_set_final, exon_start_boundary_final = get_final_exon_intervals(exon_start_list,
                                                                                       exon_start_boundaries,
                                                                                       exon_intervals_list,
            ...
            exon_end_set_final, exon_end_boundary_final = get_final_exon_intervals(exon_end_list, exon_end_boundaries,
                                                                                   exon_intervals_list,
                                                                                   nonoverlapping_gene_intervals, 'end')
            ...
            sxboundary, syboundary = create_training_set(exon_start_set_final, [0]*len(exon_start_boundary_final), gene)
    ...
    print('No. of samples in start or end training set:', len(start_training_x),
